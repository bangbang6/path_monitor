  ...
  mounted () {
    this.options = {
      ...
      radar: [{  //配置雷达图坐标系组件，只适用于雷达图
        ...
        splitNumber: 3,  //设置指示器轴的分割段数，default
        ...
        indicator: [  //配置雷达图指示器，指定雷达图中的多个变量，跟data中value对应
          { name: '医疗质量', max: 10, min: 7 }, { name: '运营效率', max: 10, min: 7 },
          { name: '持续发展', max: 10, min: 7 }, { name: '满意度', max: 10, min: 7 },
          //设置指示器的名称，最大值，标签的颜色
        ]
      }],
      ...
      series: [{
        ...
        name: '各指标得分',  //系列名称，用于tooltip的显示，图例筛选
        type: 'radar',  //系列类型: 雷达图
        ...
        data: [  //设置雷达图的数据是多变量(维度)
          {   //设置第1个数据项
            value: [9.68, 8.42, 9.12, 8.24],  //value是具体数据
            symbol: 'circle',
            symbolSize: 5,  //设置单个数据标记的大小
            //设置拐点标志样式
            itemStyle: { normal: { borderColor: '#74fbf5', borderWidth: 2, color: "#5087EC" } },
            //设置单项线条样式
            lineStyle: { normal: { color: '#74fbf5', width: 2, opacity: 1 } },
        ...
        name: '平均分',  //系列名称，用于tooltip的显示，图例筛选
        type: 'radar',  //系列类型: 雷达图
        //拐点样式，'circle'|'rect'|'roundRect'|'triangle'|'diamond'|'pin'|'arrow'|'none'
        symbol: 'circle',
        // itemStyle: {  //设置折线拐点标志的样式
        ...
        data: [  //设置雷达图的数据是多变量(维度)
          {   //设置第1个数据项
            value: [9.12, 9.04, 8.96, 9.00],  //value是具体数据
            symbol: 'circle',
            symbolSize: 5,  //设置单个数据标记的大小
            //设置拐点标志样式
            itemStyle: { normal: { borderColor: '#ed1941', borderWidth: 2, color: "#5087EC" } },
            //设置单项线条样式
            lineStyle: { normal: { color: '#ed1941', width: 2, opacity: 1 } },
            //areaStyle: {normal:{color:'red'}}  //设置单项区域填充样式
          },
          //设置第2个数据项
